...
class Instagrambot:
    ...
    def toggle_follow(self,user):
        self.nav_user(user)
        try:
            follow_list = self.driver.find_elements_by_xpath("//button[contains(text(), 'Follow')]")
            follow_list[0].click()
        except:
            unfollow = self.driver.find_elements_by_class_name('vBF20._1OSdk')
            unfollow[0].click()
            confirm = self.driver.find_element_by_class_name('aOOlW.-Cab_').click()
    ...
    def unfollow_user(self):
        try:
            time.sleep(1)
            unfollow = self.driver.find_element_by_class_name('_5f5mN.-fzfL._6VtSN.yZn4P').click()
        ...
        if check == True:
            ...
            for i in range(1,liking):
                    ...
                    # finds the button which gives the next picture 
                        ...
                        comments = ['Love it!', ' #Love', '💖🔥','love the post','Legend','Come check out outer', 'I\'m loving it', 'Finger Licking Good', 'Keep it up']
                        ...
                        if rand <= odds:
                            ...
                            while(1 == 1) and five != 5:
                                try:
                                    ...
                                    commentSection.send_keys(comments[comment_random])
...
if __name__ == '__main__':
    while odds > 100:
        try:
            odds = int(input('What percentage odds do you want for comment on post [0-100]: ')) 
        except:
           pass
